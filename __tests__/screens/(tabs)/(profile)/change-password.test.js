...
jest.mock("../../../../hooks/providers/user-provider", () => ({
    useUser: jest.fn(() => ({
        user: {id: "1", name: "TestUser", email: "test@test.com", password: "12345678"},
        login: jest.fn(),
    })),
}));
...
describe("ChangePassword", () => {
    ...
    it("should change password if actual password is correct", async () => {
        ...
        const confirmNewPasswordInput = getByPlaceholderText("Confirmar nueva contraseña");
    ...
    it("should show an error if user is logged with google", async () => {
        const useUserMock = require("../../../../hooks/providers/user-provider").useUser;
        useUserMock.mockReturnValue({
            user: { id: "1", name: "TestUser", email: "test@test.com", password: null }
        });
        const {getByPlaceholderText, getByTestId} = render(<ChangePassword />);
        const actualPasswordInput = getByPlaceholderText("Contraseña actual");
        fireEvent.changeText(actualPasswordInput, "12345678");
        ...
        const confirmNewPasswordInput = getByPlaceholderText("Confirmar nueva contraseña");
        fireEvent.changeText(confirmNewPasswordInput, "123456789");
        getUserByEmail.mockResolvedValue({email: "test@example.com", password: null});
        const changePasswordButton = getByTestId("change-password-button");
        fireEvent.press(changePasswordButton);
        expect(Alert.alert).toHaveBeenCalledWith('Error', 'Solo podrás cambiar la contraseña si no has iniciado sesión con Google.');
    });
    ...
    it("should show an error if actual password is empty", async () => {
        ...
        const confirmNewPasswordInput = getByPlaceholderText("Confirmar nueva contraseña");
        fireEvent.changeText(confirmNewPasswordInput, "123456789");
        getUserByEmail.mockResolvedValue({email: "test@example.com", password: "123456789"});
        const changePasswordButton = getByTestId("change-password-button");
        fireEvent.press(changePasswordButton);
        expect(Alert.alert).toHaveBeenCalledWith('Error', 'La contraseña actual no es correcta.');
    });
    ...
    it("should show an error if actual password is incorrect", async () => {
        ...
        const changePasswordButton = getByTestId("change-password-button");
        fireEvent.press(changePasswordButton);
        waitFor(() => expect(Alert.alert).toHaveBeenCalledWith('Error', 'La contraseña actual no es correcta.'));
    });
    ...
    it("should show an error if new password is the same as the actual", async () => {
        ...
        const changePasswordButton = getByTestId("change-password-button");
        fireEvent.press(changePasswordButton);
        waitFor(() => expect(Alert.alert).toHaveBeenCalledWith('Error', 'La nueva contraseña debe ser diferente a la actual.'));
    });
